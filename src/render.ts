import type { Topology, SystemDelta, Decision } from "./types.js";
...
function app(children: string[]): string {
  return `<div id="app">\n${children.join("\n")}\n</div>`;
}
...
function summaryView(): string {
  return `<main class="view active" id="view-summary"></main>`;
}

function graphView(): string {
  return `<main class="view" id="view-graph">
...
function cssLayout(): string {
  return `header { padding: 12px 24px; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 24px; }
...
function cssSummary(): string {
  return `#view-summary { overflow-y: auto; padding: 40px 24px; }
...
function cssDecisions(): string {
  return `.decision-section { margin-bottom: 32px; }
...
.node-decision-links-link { color: var(--accent-purple); }`;
}

function cssCode(): string {
  return `.code-file { margin-bottom: 4px; }
...
function cssGraph(): string {
  return `.graph-toolbar { padding: 8px 16px; border-bottom: 1px solid var(--border); display: flex; gap: 6px; }
...
function clientScript(): string {
  ...
  (function buildSummary() {
    ...
    const allNodes = [
      ...
      ...delta.added.map(id => ({ id, status: "added", summary: topoNodeById[id]?.description || "New component", before: null, structural: [] })),
    ...
    function renderNodeCard(n) {
      ...
      if (linked.length > 1) {
        ...
        c += linked.map(d => '<span class="node-decision-links-link">' + esc(d) + '</span>').join(', ');
        c += '</div>';
      }
      if (n.before) {
  ...
  function selectGraphNode(id) {
    ...
    if (nd && nd.files.length > 0) {
      h += '<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border)"><h4 style="font-size:11px;color:var(--text-tertiary);margin-bottom:6px">CODE</h4>';
